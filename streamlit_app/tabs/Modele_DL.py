...
def run():
    ...
    st.markdown(
        """
        Ici, vous retrouverez tout le nécessaire pour effectuer l'inférence de nos modèles, du plus basique au plus complexe, ainsi qu'afficher différentes métriques.
        """
    )
    ...
    st.markdown(
        """
        Choisissez le modèle de base utilisé, pour afficher les différentes déclinaisons.
        """
    )
    ...
    st.markdown(
        """
        Choisissez ce que vous souhaitez afficher comme informations.
        """
    )
    ...
    if options == 'Inférence image importée' and modele_base == "EfficientNet":
        st.markdown(
        """
        **Vous pouvez importer une image depuis votre ordinateur pour en faire l'inférence (peut prendre un peu de temps).**
        """
    )
        ...
        st.markdown(
        """
        **Sélectionne aléatoirement une image du dataset Test pour en afficher la prédiction.**
        """
    )
        ...
        st.markdown(
        """
        **Affiche 4 images aléatoires dans Test donc la classification a été ratée.**
        """
    )
        ...
        st.markdown(
        """
        **Affiche les moyennes des différents scores ainsi que les 20 classes aux scores les plus bas.**
        """
        )
        ...
        st.markdown(
        """
        **Affiche un/des graphique(s) représentant l'évolution de la validation_accuracy en fonction des epochs durant l'entraînement du modèle.**
        """
        )
        ...
        if modele_base == "FromScratch":
            ...
            if sous_modele == "EfficientNetB0 - 523 classes - double modèle - dataset détouré et équilibré":
                ...
                st.write("Le premier graphique corresponds au modèle entraîné sur la première partie du dataset, et la même logique s'applique pour le second graphique.")
